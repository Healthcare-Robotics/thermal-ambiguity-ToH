...
def run_exp(Material='test', sampleThickness=50, P_Gain=1e-3, I_Gain=0, D_Gain=8e-3, set_temp=29.5, cooling=False, duration=20., min_V=0.5, max_V=3.3, g1_V_setpoint=2.3):
   ...
   # force_dev_nm = '/dev/cu.teensy.s2130840' # force teensy serial number
    temp_dev_nm = '/dev/ttyACM1' # thermal teensy serial number
    ...
    while len(desired_temp) > 0 and not(rospy.is_shutdown()) and trial_index <= 1200:
        ...
        while not(rospy.is_shutdown()) and not(restart):
            ...
            if raw_temp_data== [-1]: # Hack! [-1] is code for 'reset me'
                ...
                T0 = temperature([raw_temp_data[0]], 3.3, 9930.)[0] #passive sensor
                T1 = temperature_HT10K_Steinhart([raw_temp_data[3]])[0] #active sensor
                T2 = temperature_HT10K_Steinhart([raw_temp_data[2]])[0]
            ...
            if np.mod(i,int(freq)) == 0 and i > 0:
                #Prints every time step?
                print int(t), np.round(F_data[-1],1), np.round(np.mean(T0_data[-10:]),2), np.round(np.mean(T1_data[-10:]),2), np.round(np.mean(T3_data[-10:]),2), np.round(g1_voltage_setpoint,2), np.round(g1_voltage,2), np.round(z_set,0), np.round(z,0), up, dn
           
            if z_set == up and i > up_time*freq:
                a = 0
                while(a < len(desired_temp)):
                    #print abs(np.array(G1_error)) 
            ...
            if F_data[-1] == 0 and z_set == dn and time.time() - t_down > 20:
                ...
                restart = True 
                save_data = False          
            if F_data[-1] > 2.0 and z_set == dn: # F_data[-1] > 0 and
        ...
        if save_data:
            if True:
                ...
                A = np.array([t,T0_data, T1_data, T3_data, F_data]).T
                
                #i_save = np.argmin(abs(np.array(Time_data) + time_before_contact))
                #A = A[i_save:,:]
                ut.save_pickle(A, directory + "/room_temp_alum_" + np.str(np.round(room_temp,2))+'_sensor_'+str(set_temp)+'.pkl')
                print directory + "/room_temp_alum" + str(np.round(room_temp,2))+'_sensor_'+str(set_temp)+'.pkl', "saved"
                print ' '
...
def find_PID_params():
    ...
    for i in [5e-3]: # np.linspace(1e-4, 1e-3, 10):
        ...
        print "    Experiment..."
        data = run_exp(P_Gain=8e-4, I_Gain=0, D_Gain=8e-3, set_temp=25.4, duration=60., Material='_')[2000:]
